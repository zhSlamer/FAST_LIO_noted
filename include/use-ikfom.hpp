...
//输入u 加速度a和角速度w 输入u(6维)，也就是IMU的实际测量值
MTK_BUILD_MANIFOLD(input_ikfom,
((vect3, acc))
((vect3, gyro))
);

// Eigen::Matrix<double, 12, 12> Q; Q = process_noise_cov(); 
...
Eigen::Matrix<double, 24, 1> get_f(state_ikfom &s, const input_ikfom &in)
{
	...
	for(int i = 0; i < 3; i++ ){
		res(i) = s.vel[i];							//速度（对应公式第2行）  	v 0没有加入加速度量
		res(i + 3) =  omega[i]; 					//角速度（对应公式第1行）	w 3
		res(i + 12) = a_inertial[i] + s.grav[i]; 	//加速度（对应公式第3行）	a 12
	}
	return res;
}
...
Eigen::Matrix<double, 24, 23> df_dx(state_ikfom &s, const input_ikfom &in)
{
	...
	// 论文中的顺序为 R p v bw ba g  R_IL p_IL
	// 该代码顺序为   p R R_IL p_IL v bw ba g
	...
	cov.template block<3, 3>(12, 18) = -s.rot.toRotationMatrix();					//对应公式(7)第3行第5列 
